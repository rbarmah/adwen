...
type Phase = 'loading' | 'pending' | 'ready' | 'playing' | 'submitted' | 'results';
...
export default function DuelPlayPage() {
  ...
  const fetchDuel = useCallback(async () => {
    ...
    if (status === 'completed' || data.hasPlayed) {
      ...
      setPhase('pending');
    } else if (['accepted', 'in_progress'].includes(status) || (status === 'pending' && data.isChallenger)) {
      setPhase('ready');
    } else {
      setPhase('results');
    }
  ...
  const startQuiz = async () => {
    // If challenger and duel is pending, they can start playing immediately
    // Otherwise get the questions
  ...
  const handleAnswer = (chosenIndex: number, isCorrect: boolean) => {
    ...
    const answer = {
      itemId: item.id,
      chosenIndex,
      isCorrect: chosenIndex === -1 ? false : isCorrect,
      latencyMs,
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '20px 16px' }}>
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          {item.options.map((opt, i) => {
            ...
            return (
              <button key={i} onClick={() => { if (!showFeedback) handleAnswer(i, isCorrect); }}
